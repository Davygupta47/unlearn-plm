...
dataset_path_dict = {
    ...
    "tofu_forget": {"name":"forget10", "split":"train"},
    "tofu_retain": {"name":"retain90", "split":"train"},
}
...
def save_tokenized_dataset(
    ...
    if dataset_name in dataset_path_dict.keys():
        ...
        # import pdb
        # pdb.set_trace()
        if dataset_name.startswith("tofu"):
            raw_dataset = load_dataset("locuslab/TOFU", dataset_path["name"], split=dataset_path["split"])
            def map_tofu(example):
                text = example.get("question", "") + "\n" + example.get("answer", "")
                return {"text": text}
            raw_dataset = raw_dataset.map(map_tofu)
        else:
