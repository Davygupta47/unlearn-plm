...
if __name__ == "__main__":
    ...
    domains= ["arxiv", "github", "tofu"]
    for negative_domain in domains:
        if negative_domain == "tofu":
            negative_train = load_dataset("locuslab/TOFU", "forget10", split="train")
            negative_train = negative_train.map(lambda ex: {"text": ex["question"] + "\n" + ex["answer"]})
        else:
        ...
        for positive_domain in ["general", negative_domain]:
            if positive_domain == "tofu":
                positive_train = load_dataset("locuslab/TOFU", "retain90", split="train")
                positive_train = positive_train.map(lambda ex: {"text": ex["question"] + "\n" + ex["answer"]})
            else:
